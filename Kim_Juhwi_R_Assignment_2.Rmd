---
title: 'Kim_Juhwi_R_Assignment_2'
output:
...
dpois(x, lambda=p*n)
ppois(x, lambda=p*n)
#dpois and ppois have same value of 0.006737947
dbinom(x, n, p)
pbinom(x, n, p)
#dbinom and pbinom also result in a same value: 0.005920529
```
...
sum(dpois(0:x, lambda=p*n))
ppois(x, lambda=p*n)
#dpois and ppois have same value of 0.6159607
sum(dbinom(0:x, n, p))
pbinom(x, n, p)
#dbinom and pbinom also result in a same value: 0.6159991
...
lambda = n * p
x = 19
c = sprintf("dpois: %.5f vs dbinom: %.5f", sum(dpois(0:x, lambda)), sum(dbinom(0:x, n, p)))
d = sprintf("ppois: %.5f vs pbinom: %.5f", ppois(x, lambda), pbinom(x, n, p))
cat(c,d)
...
par(mfrow=c(1,2))
barplot(dp, main ="Poisson Distribution", col="pink", xlab = "Outcome", ylab ="Probability")
barplot(db, main = "Binomial Distribution", col = "purple", xlab = "Outcome", ylab = "Probability")
...
```{r test1ci}
x <- c(0, 1, 2, 3, 4, 5, 6)
prob.x <- c(0.215, 0.230, 0.240, 0.182, 0.130, 0.003, 0.001)
mu <- sum(x * prob.x)
x.var <- sum((x - mu)^2 * prob.x)
u = sprintf("The expected value: %.2f", mu)
v = sprintf("The variance: %.2f", x.var)
cat(u,v)
...
```{r test1cii}
cumprob <- cumsum(prob.x)
cumprob
plot(x, cumprob, main="Cumulative probabilites based on cumalative outcome", xlab="Outcome", ylab="Cumulative probability")
lines(x, cumprob)
points(median(x), median(cumprob), col="Red")
...
summary(faithful)
hist(faithful$waiting, main="Waiting Times", xlab= "Waiting Time", ylab="Frequency", col="Yellow" )

waitover70 <- subset(faithful, waiting > 70)
erupt3min <- subset(waitover70, eruptions < 3)
cond_prob = nrow(erupt3min)/nrow(waitover70)
cond_prob
```
...
```{r test2ai}
subsetfaithful = subset(subset(faithful, waiting != erupt3min$waiting), eruptions != erupt3min$waiting)

plot(subsetfaithful$waiting, subsetfaithful$eruptions, col="green", main="Waiting time > 70 minutes | Eruptions < 3 minutes", xlab="Waiting Time", ylab="Eruption Time")

abline(h=3, col="purple")
abline(v=70, col="purple")
points(erupt3min$waiting, erupt3min$eruptions, col="red", pch=19)

```

(ii) What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: (There seems to be a postive correlation between eruption and wait time. The longer you wait, you will see longer eruption.)    ***
...
```{r test2b}
waitmatrix <- matrix(faithful$waiting, ncol=2,  byrow=TRUE)

plot(x = waitmatrix[, 1], y = waitmatrix[, 2], col="violet", main = "Faithful Observations", 
...
```{r test2c}
cor.test(waitmatrix[,1],waitmatrix[,2],alternative = "two.sided",method = "kendall", conf.level = 0.95)
```
...
chicksubset <- subset(ChickWeight,Time == 21 & Diet == "1" | Diet == "3")
head(chicksubset, n=6)
...
```{r test3a}
result1 <- subset(chicksubset, Diet == "1")
result3 <- subset(chicksubset, Diet == "3")
...
before <- subset(ChickWeight, Time == 20 & Diet == "3")$weight
after <- subset(ChickWeight,Time == 21 & Diet == "3")$weight
...
```{r test3c}
plot(before, after, col="violet", main = "Before ~ After",
...
cat("Test statistic value: ", t, "\n")
cat("Critical value: ", cv, "\n")
cat("P-value: ", pv, "\n")
cat("Confidence interval: (", cminus, ",", cplus, ")")
...
hist(Nile, freq = FALSE, col = "blue", xlab = "Flow",
     main = "Nile River Flows, 1871 to 1970")
...
par(mfrow = c(1, 2))
qqnorm(Nile, col="violet", main = "Normal Q-Q Plot (Nile)")
qqline(Nile, col = "steelblue")
...
for (i in 1:1000) {sample1[i] <- mean(sample(Nile, size = 16, replace =TRUE))}

sample2 <- rep(1,1000, 0)
for (i in 1:1000) {sample2[i] <- mean(sample(Nile, size = 64, replace =TRUE))}

sample_mean <- c(mean(sample1), mean(sample2))
sample_sd <- c(sd(sample1), sd(sample2))
sample_var <- c(var(sample1), var(sample2))

row_label <- c("Sample 1", "Sample 2")
col_label <- c("Mean", "Sample SD", "Sample Variance")

matrix(c(sample_mean, sample_sd, sample_var), nrow=2, ncol=3, dimnames = list(row_label, col_label))
...
hist(sample1, freq = FALSE, col = "pink", xlab = "Flow", main = "Sample 1",
     xlim = xlim,ylim = ylim)
...
hist(sample2, freq = FALSE, col = "green", xlab = "Flow", main = "Sample 2",
     xlim = xlim,ylim = ylim)
curve(dnorm(x, mean = m, sd = std), col = "dark green", lwd = 2, add = TRUE)
...
hist(warpbreaks$breaks, main = "Histogram of warpbreaks", xlab = "Number of Breaks", col="light pink")
abline(v=median_breaks, col="purple", lty=2, lwd=2)
...
#Testing the null hypothesis at the 5%, the p-value is smaller than the alpha. Therefore, we can conclude that the variables are not independent and that we reject the null hypothesis.
```
